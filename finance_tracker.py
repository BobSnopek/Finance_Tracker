...
class FinanceApp:
    ...
    def setup_profile(self):
        ...
        # Yield Logic - Fixed to handle invalid input properly
        raw_yield = input("Expected Annual Return % (e.g. 7) [Default: 5%]: ").strip()
        if raw_yield and re.search(r'\d', raw_yield):
            y_val = self.cleaner.clean_to_float(raw_yield) / 100
        else:
            y_val = 0.05  # Default 5%
        
        # Inflation Logic - Fixed to handle invalid input properly
        raw_inf = input("Expected Annual Inflation % (e.g. 3) [Default: 4.5%]: ").strip()
        if raw_inf and re.search(r'\d', raw_inf):
            inf_val = self.cleaner.clean_to_float(raw_inf) / 100
        else:
            inf_val = 0.045  # Default 4.5%
    ...
    def run_report(self):
        ...
        # Visualization - Fixed plotting logic
        plt.figure(figsize=(12, 6))
        
        # Split data for coloring based on Status field
        debt_df = df[df['Status'] == "Debt"]
        inv_df = df[df['Status'] == "Investing"]

        # Plot debt repayment phase
        if not debt_df.empty:
            plt.plot(debt_df['Year'], debt_df['NetWorth'], color='#e74c3c', label='Debt Repayment', linewidth=3)
        
        # Plot wealth building phase with proper connection
        if not inv_df.empty:
            if not debt_df.empty:
                # Connect from the last debt point for smooth transition
                transition_idx = debt_df.index[-1]
                plot_df = df.loc[transition_idx:]
            else:
                # No debt phase, plot entire investment period
                plot_df = inv_df
            plt.plot(plot_df['Year'], plot_df['NetWorth'], color='#2ecc71', label='Wealth Building', linewidth=3)

        plt.axhline(0, color='black', linestyle='--', alpha=0.3)
        plt.title(f"10-Year Financial Path ({p['currency']})\nNominal: {p['annual_yield']*100:.1f}% | Inflation: {p['inflation']*100:.1f}%", fontsize=14)
        plt.xlabel("Years")
